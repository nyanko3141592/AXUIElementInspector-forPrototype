        ...
        for attributeName in attributeNames {
            ...
            if attributeValueError == .success {
                ...
                description.append(
                    NSAttributedString(
                        string:
                            "\(String(repeating: " ", count: (level + 1) * 2))\(attributeName): ",
                        attributes: [.font: NSFont.monospacedSystemFont(ofSize: 11, weight: .bold)]
                    )
                )
                ...
                if AXValueGetType(value) == .cgPoint {
                    ...
                    description.append(
                        NSAttributedString(
                            string:
                                "\(NSStringFromRange(NSRange(location: r.location, length: r.length)))\n",
                            attributes: [
                                .font: NSFont.monospacedSystemFont(ofSize: 11, weight: .regular)
                            ]
                        )
                            ...
                            if let children = attributeValue as? [AXUIElement] {
                                ...
                                if level < 1 {
                                    ...
                                    for child in children {
                                        let subDescription = inspect(
                                            element: child, level: level + 2)
