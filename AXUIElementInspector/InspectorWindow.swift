...
class InspectorWindow: NSPanel {
    ...
    private let inspectButton = NSButton(
        image: NSImage(named: "crosshair")!, target: nil, action: nil)
    ...
    init() {
        ...
        NSLayoutConstraint.activate([
            activeApplicationNameLabel.topAnchor.constraint(
                equalTo: inspectedElementSectionLabel.topAnchor),
            activeApplicationNameLabel.trailingAnchor.constraint(
                equalTo: inspectButton.leadingAnchor, constant: -12),
        ])
        // Auto Layoutを設定
        NSLayoutConstraint.activate([
            inspectedElementSectionLabel.topAnchor.constraint(
                equalTo: rootView.topAnchor, constant: 12),
            inspectedElementSectionLabel.leadingAnchor.constraint(
                equalTo: rootView.leadingAnchor, constant: 12),
            inspectedElementSectionLabel.trailingAnchor.constraint(
                equalTo: inspectButton.leadingAnchor, constant: -12),

            elementTitleLabel.topAnchor.constraint(
                equalTo: inspectedElementSectionLabel.bottomAnchor, constant: 2),
            elementTitleLabel.leadingAnchor.constraint(
                equalTo: rootView.leadingAnchor, constant: 12),
            elementTitleLabel.trailingAnchor.constraint(
                equalTo: inspectButton.leadingAnchor, constant: -8),
